...
use influx_db_client::{Client, Value, Precision, Point};

fn main() {
    let influx_username = std::env::var("HS110_INFLUX_USERNAME").expect("Could not read environment variable: HS110_INFLUX_USERNAME");
    let influx_password = std::env::var("HS110_INFLUX_PASSWORD").expect("Could not read environment variable: HS110_INFLUX_PASSWORD");
    let influx_database = std::env::var("HS110_INFLUX_DATABASE").expect("Could not read environment variable: HS110_INFLUX_DATABASE");

    let influx_host = std::env::var("HS110_INFLUX_HOST").expect("Could not read environment variable: HS110_INFLUX_HOST");
    ...
    let t = thread::spawn(move || {
        loop {
            let sysinfo = plug.get_sysinfo().unwrap().
                system.unwrap().
                get_sysinfo;
